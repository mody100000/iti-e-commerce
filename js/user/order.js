...
const listUserOrders = async () => {
  ...
  const spinner = createASpinner("track-orders-wrapper");
  spinner.show();
  ...
  if (userOrdersSnapshot.exists) {
    ...
    const ordersWrapper = $(`#track-orders-wrapper`);
    Object.entries(orders).forEach(([orderRef, order]) => {
      let total = 0;
      ordersWrapper.append(
        $(`
        <div class="accordion" id="order-according-${orderRef}">
        ...
        <div
          ...
          data-bs-parent="#order-according-${orderRef}"
        >
